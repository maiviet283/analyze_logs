...
class ZeekLogFetcher(BaseLogFetcher):
    """
    Fetcher for Zeek logs from Elasticsearch.
    """
    def __init__(self):
        super().__init__(index_pattern="zeek-logs-*")


    def detect_scanner(self, seconds=20, threshold_requests=50):
        """
        Phát hiện IP gửi nhiều request trong khoảng thời gian nhất định.
        seconds: window thời gian (giây)
        threshold_requests: ngưỡng request để xem là đáng ngờ
        Trả về danh sách IP vượt ngưỡng cùng số request thực tế.
        """
        df = self.fetch_logs(seconds=seconds, size=2000)
        ...
        if not ip_candidates:
            print("[DEBUG] Columns after flatten:", df.columns.tolist())
        ...
        # Lấy thời gian log đầu tiên của mỗi IP để hiển thị
        alert_times = {}
